...
def _scoped_text(tex: str, reason=None, baseline=None) -> str:
    return " ".join(
        tex[r.start:r.end] for r in review_scope(tex, baseline) if reason is None or r.reason == reason
    )


def test_scope_is_ai_sections_only():
    tex = _report()
    scoped = _scoped_text(tex)
...
def test_edited_fixed_text_enters_scope_but_references_never_do():
    baseline = _report()
    edited = baseline.replace("Texto fixo.", "Texto fixo alterado pelo analista.").replace(
        "SILVA, J. Obra. 2020.", "SILVA, J. Obra revisada. 2020."
    )
    scoped = _scoped_text(edited, reason="editado", baseline=baseline)
