...
- `probe_query_term` é nova: guarda os termos extraídos por NLP local (spaCy + KeyBERT + TF-IDF, não é IA) a partir dos documentos de uma probe query (Amostragem de Termos), com `selected` marcando os termos escolhidos pelo usuário pra montar a query final. Só populada em linhas de probe (`tipo=None`); sem tabela compartilhada + junção como `patent`/`article`, pois um termo só faz sentido no contexto da probe query que o gerou.
